...
docker rm -f vellum-op-proposer vellum-op-batcher vellum-op-node vellum-op-geth >/dev/null 2>&1 || true
docker volume rm vellum_op_geth_data >/dev/null 2>&1 || true
docker network rm vellum-devnet >/dev/null 2>&1 || true
rm -rf "$DEVNET_DIR"
